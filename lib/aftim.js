...
async function queryAftim(query, variables = {}) {
  const res = await fetch(AFTIM_URL, {
    ...
    // IMPORTANTE: Aseguramos que el cuerpo sea un JSON limpio
    body: JSON.stringify({ 
      query: query.trim(), 
      variables: variables 
    }),
...
async function getFacturasRango(fi, ff) {
  // Corregimos la definición de tipos en el query para evitar el error de AST
  const query = `
...
export async function obtenerTodosLosDatos() {
  ...
  // Ejecución secuencial para no saturar la API
  ...
  detMes.forEach(d => {
    ...
    if (!productosMap[nombre]) {
      productosMap[nombre] = { nombre, cantidad: 0, monto_usd: 0 };
    }
  ...
  return {
    ...
    ventasProductos: {
      mes: Object.values(productosMap)
