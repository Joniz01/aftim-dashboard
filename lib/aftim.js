...
async function queryAftim(query, variables = {}) {
  const res = await fetch(AFTIM_URL, {
    ...
    // Forzamos el envío de un JSON plano y limpio
    body: JSON.stringify({ 
...
async function getFacturasRango(fi, ff) {
  // Definición explícita de tipos para evitar el error de AST Node
  const query = `
    query($fi: DateOrDateTime, $ff: DateOrDateTime) {
      getFacturas(
        limit: 1000, 
        filter: { 
          anulada: { eq: 0 }, 
...
async function getVentasProductos(fi, ff) {
  ...
  `;
  const data = await queryAftim(query, { fi, ff });
  return (data?.getDetDetalles || []).filter(d => d.Factura?.anulada === 0);
}

export async function obtenerTodosLosDatos() {
  const f = getFechas();

  // Ejecución uno por uno para asegurar que no se bloquee el canal
